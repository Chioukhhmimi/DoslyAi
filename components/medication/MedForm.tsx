...
import { View, Text, TextInput, Switch, TouchableOpacity, ScrollView, Platform, StyleSheet } from 'react-native';
import { SafeAreaView } from 'react-native-safe-area-context';
import DateTimePicker, { DateTimePickerEvent } from '@react-native-community/datetimepicker';
import { useTranslation } from 'react-i18next';
import { format } from 'date-fns';
import { Medication, MedicationSchedule, MedicationType, NewMedication } from '@store/medicationStore';
...
const PILL_COLORS = ['#6366F1','#EC4899','#F97316','#10B981','#EAB308','#06B6D4','#EF4444','#8B5CF6','#84CC16','#F59E0B'];
...
export function MedForm({ initialValues, onSubmit, onCancel, profileId }: MedFormProps) {
  ...
  const [schedule, setSchedule] = useState<MedicationSchedule>(initialValues?.schedule ?? DEFAULT_SCHEDULE);
            ...
            <View style={styles.doseRow}>
              <View style={[styles.stepper, !!errors.doseQuantity && styles.inputError]}>
                ...
                <TextInput
                  ...
                  value={doseQuantity}
                  onChangeText={(v) => { setDoseQuantity(v); setErrors((e) => ({ ...e, doseQuantity: '' })); }}
                  placeholder="1"
                  placeholderTextColor={Colors.textDisabled}
              ...
              <TextInput
                ...
                value={unit}
                onChangeText={(v) => { setUnit(v); setErrors((e) => ({ ...e, unit: '' })); }}
                placeholder="mg"
                placeholderTextColor={Colors.textDisabled}
              />
            </View>
            ...
            <View style={styles.unitPresets}>
              {UNIT_PRESET_KEYS.map((key) => {
                ...
                return (
                  <TouchableOpacity
                    key={key}
                    onPress={() => { setUnit(key); setErrors((e) => ({ ...e, unit: '' })); }}
                    style={[styles.unitChip, unit === key && styles.unitChipActive]}
                  >
                    <Text style={[styles.unitChipText, unit === key && styles.unitChipTextActive]}>{label}</Text>
                  </TouchableOpacity>
                );
              })}
            </View>
            ...
            <View style={styles.colorRow}>
              {PILL_COLORS.map((c) => (
                <TouchableOpacity
                  ...
                  style={[styles.colorSwatch, { backgroundColor: c }, pillColor === c && styles.colorSwatchActive]}
                />
              ))}
            </View>
        ...
        {step === 3 && (
          <View>
            ...
            <View style={styles.toggleRow}>
              ...
              <Switch
                value={ongoing}
                onValueChange={(v) => { setOngoing(v); if (v) setEndDate(null); setErrors({}); }}
            ...
            {Platform.OS === 'ios' && (
              <BottomSheet visible={showStartPicker} onClose={() => setShowStartPicker(false)} showHandle={false}>
                <View style={styles.pickerHeader}>
                  <TouchableOpacity onPress={() => setShowStartPicker(false)}>
                    <Text style={styles.pickerDone}>{t('common.done')}</Text>
                  </TouchableOpacity>
                </View>
                <DateTimePicker value={startDate} mode="date" display="spinner" onChange={onStartDateChange} style={styles.picker} />
              </BottomSheet>
            )}

            {Platform.OS === 'android' && showEndPicker && (
              <DateTimePicker value={endDate ?? new Date()} mode="date" display="default" minimumDate={startDate} onChange={onEndDateChange} />
            )}
            {Platform.OS === 'ios' && (
              <BottomSheet visible={showEndPicker} onClose={() => setShowEndPicker(false)} showHandle={false}>
                <View style={styles.pickerHeader}>
                  <TouchableOpacity onPress={() => setShowEndPicker(false)}>
                    <Text style={styles.pickerDone}>{t('common.done')}</Text>
                  </TouchableOpacity>
                </View>
                <DateTimePicker value={endDate ?? new Date()} mode="date" display="spinner" minimumDate={startDate} onChange={onEndDateChange} style={styles.picker} />
              </BottomSheet>
            )}
          </View>
        )}
        ...
        {step === 4 && (
          <View style={styles.review}>
            ...
            <View style={styles.reviewCard}>
              ...
              <ReviewRow label={t('medication.form.typeLabel')} value={t(`medication.types.${type}`)} />
...
const styles = StyleSheet.create({
  ...
  progressBar:      { height: 4, backgroundColor: Colors.border, borderRadius: 2, margin: Spacing.md, marginBottom: 0 },
  progressFill:     { height: 4, backgroundColor: Colors.primary, borderRadius: 2 },
  stepLabel:        { fontSize: FontSize.xs, color: Colors.textSecondary, textAlign: 'center', marginTop: 4, marginBottom: Spacing.md },
  ...
  stepper:          { flex: 0.45, flexDirection: 'row', alignItems: 'center', borderWidth: 1, borderColor: Colors.border, borderRadius: Radius.sm, backgroundColor: Colors.surface, overflow: 'hidden' },
  stepperBtn:       { width: 40, height: 48, alignItems: 'center', justifyContent: 'center', backgroundColor: Colors.surfaceSubtle },
  stepperBtnText:   { fontSize: FontSize.lg, fontWeight: '600', color: Colors.primary },
  stepperInput:     { flex: 1, height: 48, fontSize: FontSize.md, color: Colors.textPrimary, textAlign: 'center' },
  doseUnitInput:    { flex: 0.55 },
  unitPresets:      { flexDirection: 'row', flexWrap: 'wrap', gap: Spacing.xs, marginTop: Spacing.xs, marginBottom: Spacing.sm },
  unitChip:         { paddingVertical: 4, paddingHorizontal: Spacing.sm, borderRadius: Radius.full, borderWidth: 1, borderColor: Colors.border, backgroundColor: Colors.surface },
  ...
  chip:             { paddingVertical: 6, paddingHorizontal: Spacing.sm, borderRadius: Radius.full, borderWidth: 1, borderColor: Colors.border, backgroundColor: Colors.surface },
  ...
  review:           {},
  reviewCard:       { backgroundColor: Colors.surface, borderRadius: Radius.md, padding: Spacing.md, borderWidth: 1, borderColor: Colors.border },
  nav:              { flexDirection: 'row', gap: Spacing.sm, padding: Spacing.md, borderTopWidth: 1, borderTopColor: Colors.border },
  navBtn:           { flex: 1 },
  pickerHeader:     { flexDirection: 'row', justifyContent: 'flex-end', paddingBottom: Spacing.md, borderBottomWidth: 1, borderBottomColor: Colors.border },

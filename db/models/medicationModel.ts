...
export async function dbUpdateMedication(id: string, data: Partial<Medication>): Promise<void> {
  ...
  if (data.doseQuantity !== undefined)         { fields.push('dose_quantity = ?');           values.push(data.doseQuantity); }
  if (data.unit !== undefined)                 { fields.push('unit = ?');                    values.push(data.unit); }
  if (data.type !== undefined)                 { fields.push('type = ?');                    values.push(data.type); }
  if (data.schedule !== undefined)             { fields.push('schedule = ?');                values.push(JSON.stringify(data.schedule)); }
...
export async function dbInsertIntakeRecord(record: IntakeRecord): Promise<void> {
  ...
  await db.runAsync(
    `INSERT OR REPLACE INTO intake_records (id, medication_id, profile_id, scheduled_at, taken_at, skipped, notes)
     VALUES (?, ?, ?, ?, ?, ?, ?)`,
    [record.id, record.medicationId, record.profileId, record.scheduledAt, record.takenAt ?? null, record.skipped ? 1 : 0, record.notes ?? null]
  );
}
...
export async function dbGetAllIntakeRecords(): Promise<IntakeRecord[]> {
  const db = await getDatabase();
  const rows = await db.getAllAsync<{
    id: string; medication_id: string; profile_id: string;
    scheduled_at: string; taken_at: string | null; skipped: number; notes: string | null;

...
export async function dbUpdateProfile(id: string, data: Partial<Profile>): Promise<void> {
  ...
  if (data.allergies !== undefined) {
    ...
    values.push(data.allergies ? JSON.stringify(data.allergies) : null);
  }
  ...
  if (data.doctorPhone !== undefined) {
    fields.push('doctor_phone = ?');
    values.push(data.doctorPhone ?? null);
  }
  ...
  if (data.emergencyPhone !== undefined) {
    ...
    values.push(data.emergencyPhone ?? null);
  }
...
export async function dbDeleteAllProfiles(): Promise<void> {
  const db = await getDatabase();
